...
npx @claude-flow/cli@3.38.12 github release-create \
  --version "2.0.0" \
  --build-artifacts \
...
npx @claude-flow/cli@3.38.12 sparc pipeline "Release v2.0.0 with full validation"
```
...
npx @claude-flow/cli@3.38.12 github changelog \
  --prs "$PRS" \
  --commits "$COMMITS" \
  --from v1.0.0 \
  --to HEAD \
...
npx @claude-flow/cli@3.38.12 github version-suggest \
  --current v1.2.3 \
  --analyze-commits \
  --check-compatibility \
  --suggest-pre-release
...
```bash
# Multi-platform build coordination
npx @claude-flow/cli@3.38.12 github release-build \
...
npx @claude-flow/cli@3.38.12 github release-test \
...
npx @claude-flow/cli@3.38.12 github release-deploy \
...
npx @claude-flow/cli@3.38.12 github multi-release \
...
npx @claude-flow/cli@3.38.12 github emergency-release \
  --issue 789 \
  --severity critical \
...
npx @claude-flow/cli@3.38.12 github release-validate \
  --checks "
    version-conflicts,
...
npx @claude-flow/cli@3.38.12 github compat-test \
...
npx @claude-flow/cli@3.38.12 github performance-test \
  --baseline v1.9.0 \
  --candidate v2.0.0 \
...
npx @claude-flow/cli@3.38.12 github release-monitor \
  --version v2.0.0 \
...
npx @claude-flow/cli@3.38.12 github release-analytics \
  --version v2.0.0 \
  --compare-with v1.9.0 \
...
npx @claude-flow/cli@3.38.12 github rollback-config \
  --triggers '{
    "error-rate": ">5%",
    "latency-p99": ">1000ms",
    "availability": "<99.9%",
...
npx @claude-flow/cli@3.38.12 github release-security \
  --scan-dependencies \
  --check-secrets \
  --audit-permissions \
  --sign-artifacts \
...
npx @claude-flow/cli@3.38.12 github release-compliance \
    ...
    steps:
      ...
      - name: Initialize Release Swarm
        run: |
          ...
          npx claude-flow@3.38.12 swarm init --topology hierarchical
      ...
      - name: Generate Release Changelog
        run: |
          ...
          CHANGELOG=$(npx claude-flow@3.38.12 github changelog \
      ...
      - name: Build Release Artifacts
        run: |
          ...
          npx claude-flow@3.38.12 github release-build \
            --platforms "linux,macos,windows" \
            --architectures "x64,arm64" \
            --parallel
      ...
      - name: Security Scan
        run: |
          ...
          npx claude-flow@3.38.12 github release-security \
            --scan-dependencies \
            --check-secrets \
            --sign-artifacts
      ...
      - name: Post-Release Validation
        run: |
          ...
          # Validate deployment
          npx claude-flow@3.38.12 github release-validate \
      ...
      - name: Monitor Release
        run: |
          ...
          npx claude-flow@3.38.12 github release-monitor \
            --version ${{ github.ref_name }} \
            --duration 1h \
            --alert-on-errors &
```
  ...
  emergency-hotfix:
    ...
    steps:
      ...
      - name: Emergency Release
        run: |
          npx claude-flow@3.38.12 github emergency-release \
...
npx claude-flow@3.38.12 diagnostic-run \
  --component build \
  --verbose
...
npx claude-flow@3.38.12 github compat-test \
...
npx claude-flow@3.38.12 github rollback \
  --to-version v1.9.9 \
...
npx claude-flow@3.38.12 github release-analytics \
  --version v2.0.0 \
  --identify-issues
```
...
npx claude-flow@3.38.12 github version-sync \
