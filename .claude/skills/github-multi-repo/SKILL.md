...
npx @claude-flow/cli@3.38.12 skill run github-multi-repo init \
...
npx @claude-flow/cli@3.38.12 skill run github-multi-repo sync \
...
npx @claude-flow/cli@3.38.12 skill run github-multi-repo optimize \
  --analyze-structure \
...
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo microservices \
...
### 2. Library Updates
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo lib-update \
...
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo org-policy \
...
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo dashboard \
  --port 3000 \
...
### Dependency Graph
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo dep-graph \
  --format mermaid \
  --include-agents \
  --show-data-flow
```
...
### Health Monitoring
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo health-check \
  --repos "org/*" \
...
### Caching Strategy
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo cache-strategy \
  --analyze-patterns \
...
### Parallel Execution
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo parallel-optimize \
...
### Resource Pooling
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo resource-pool \
  --share-agents \
  --distribute-load \
  --monitor-usage
```
...
### Connectivity Issues
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo diagnose-connectivity \
  --test-all-repos \
  --check-permissions \
  --verify-webhooks
```
...
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo debug-memory \
  --check-consistency \
  --identify-conflicts \
  --repair-state
```
...
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo perf-analysis \
  --profile-operations \
...
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo queue \
  --backend redis \
  --workers 10 \
  --priority-routing \
  --dead-letter-queue
...
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo test \
  --setup-test-env \
  --link-services \
  --run-e2e \
  --tear-down
...
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo to-monorepo \
  --analyze-repos \
  --suggest-structure \
  --preserve-history \
...
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo fullstack-update \
...
```bash
npx @claude-flow/cli@3.38.12 skill run github-multi-repo cross-team \
