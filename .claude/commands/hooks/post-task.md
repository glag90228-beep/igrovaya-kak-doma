...
```bash
npx @claude-flow/cli@3.38.12 hook post-task -t "api-refactor" --analyze-performance --generate-report
```

### Memory storage

```bash
npx @claude-flow/cli@3.38.12 hook post-task -t "bug-fix-123" --store-decisions --export-learnings
```

### Quick cleanup

```bash
npx @claude-flow/cli@3.38.12 hook post-task -t "minor-update" --analyze-performance false
```
...
npx @claude-flow/cli@3.38.12 hook post-task --task-id "your-task-id" --analyze-performance true
```
