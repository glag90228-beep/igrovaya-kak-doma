...
#### Using CLI (Fallback)
```bash
npx @claude-flow/cli@3.38.12 swarm "research AI trends" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel
...
#### Using CLI (Fallback)
```bash
npx @claude-flow/cli@3.38.12 swarm "build REST API" \
...
#### Using CLI (Fallback)
```bash
npx @claude-flow/cli@3.38.12 swarm "analyze codebase" \
  --strategy analysis \
  --mode mesh \
  --parallel \
  --timeout 300
