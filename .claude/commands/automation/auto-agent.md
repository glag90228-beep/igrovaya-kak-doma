...
```bash
npx @claude-flow/cli@3.38.12 auto agent [options]
```
...
```bash
npx @claude-flow/cli@3.38.12 auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
npx @claude-flow/cli@3.38.12 auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
npx @claude-flow/cli@3.38.12 auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
npx @claude-flow/cli@3.38.12 auto agent -t "Fix bug in login" -s minimal
```
