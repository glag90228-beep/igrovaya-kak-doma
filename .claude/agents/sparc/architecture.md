...
hooks:
  pre: |
    ...
    SIMILAR_ARCH=$(npx claude-flow@3.38.12 memory search-patterns "architecture: $TASK" --k=5 --min-reward=0.85 2>/dev/null || echo "")
    if [ -n "$SIMILAR_ARCH" ]; then
      echo "📚 Found similar system architecture patterns"
      npx claude-flow@3.38.12 memory get-pattern-stats "architecture: $TASK" --k=5 2>/dev/null || true
    fi
