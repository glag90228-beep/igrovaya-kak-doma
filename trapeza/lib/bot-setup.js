 ...
 *
 * Руками в BotFather остаётся то, чего нет в Bot API:
 *
 *   • аватар — /setuserpic;
 *   • ГЛАВНОЕ мини-приложение — /mybots → бот → Bot Settings → Configure
 *     Mini App → Enable Mini App, и туда тот же адрес, что в WEBAPP_URL.
 *     Именно оно даёт кнопку «Открыть» в СПИСКЕ ЧАТОВ и в профиле бота.
 *
 * Кнопку меню ниже с ним путают, а это разные вещи и разные места. Кнопка
 * меню (setChatMenuButton) живёт слева от поля ввода и видна, только когда
 * чат уже открыт. Кнопка главного мини-приложения видна в списке чатов —
 * то есть до того, как человек вошёл в переписку. Поставить её через API
 * нельзя ни одним методом, поэтому здесь её и нет.
 */
