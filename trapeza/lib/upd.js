...
function buildUpdHtml({ org, cp, doc }) {
  ...
  const sfHead = status === 1 ? `
    <table class="sf">
      ...
      ${/*
         * Строка 5а «Документ об отгрузке» в бланке есть с 1 июля 2021 года
         * (постановление Правительства № 534 от 02.04.2021, меняющее № 1137),
         * а мы её не печатали вовсе. Строки в бланке нет — форма не соблюдена,
         * и придираться к этому будет не наш пользователь, а инспекция его
         * покупателя, когда тот заявит вычет.
         *
         * У УПД она заполняется им же самим: документ об отгрузке и есть этот
         * бланк, отдельной накладной к нему не бывает (письмо ФНС от 17.06.2021
         * № ЗГ-3-3/4368@). Поэтому номер и дата — свои, а «№ п/п» перечисляет
         * строки таблицы, которых этот документ касается: все.
         */''}
      <tr><td class="k">Документ об отгрузке <span class="n">(5а)</span></td>
          <td colspan="3">№ п/п ${items.length > 1 ? `1-${items.length}` : '1'}
            № ${esc(doc.number)} от ${ru(doc.date)}</td></tr>
      ${/*
         * Строка 5б обязательна в бланке с 1 апреля 2026 года (постановление
         * Правительства № 26 от 23.01.2026, меняющее № 1137). Заполняется при
         * отгрузке в счёт ранее полученного аванса: номер и дата авансового
         * счёта-фактуры, несколько — через «;». Аванса не было — остаётся
         * пустой, и это нормальное, а не забытое состояние.
         *
         * Значение приходит извне (doc.advDoc), подставляет его doc-service
         * по открытым авансам этого контрагента. Строку печатаем всегда: её
         * отсутствие в бланке — несоответствие форме, а пустая строка формой
         * прямо предусмотрена. Именно пустая: прочерк «—» здесь означал бы
         * «заполнено, аванса нет», и это расходилось бы с самим правилом,
         * записанным двумя строками выше.
         */''}
      <tr><td class="k">К счёту-фактуре на аванс <span class="n">(5б)</span></td>
          <td colspan="3">${esc(doc.advDoc || '')}</td></tr>
  ...
  const body = `
    ...
    <p class="small muted">Строки в круглых скобках — счёта-фактуры (постановление № 1137
       в редакции постановления № 26 от 23.01.2026), в квадратных — передаточного
       документа (письмо ФНС ММВ-20-3/96@).</p>
