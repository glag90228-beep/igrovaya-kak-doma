 ...
 *
 * Платёжку подписывают в банке живой подписью. Факсимиле на договоре по
 * ст. 160 ГК действительно, только если стороны заранее договорились об
 * этом письменно, — по умолчанию так считать нельзя.
 *
 * Счета-фактуры — авансовый и корректировочный — здесь же, и это не
 * перестраховка. По п. 6 ст. 169 НК счёт-фактуру подписывают руководитель и
 * главный бухгалтер либо уполномоченные лица; Минфин последовательно считает,
 * что факсимильная подпись этому требованию не отвечает (письма от 10.04.2019
 * № 03-07-14/25364, от 27.08.2015 № 03-07-09/49478), и Президиум ВАС с этим
 * согласился (постановление от 27.09.2011 № 4134/11). Последствие несёт не
 * тот, кто поставил печатку, а покупатель: ему откажут в вычете, и узнает он
 * об этом от своей инспекции.
 */
const NEVER = ['pp', 'dog', 'avans', 'ksf'];

/**
 * Ставится ли факсимиле на документ такого типа.
 *
 * @param {string} scope настройка пользователя
 * @param {string} docType тип документа
 * @param {object} [doc] сам документ — у УПД смотрим статус
 */
function appliesTo(scope, docType, doc) {
  if (NEVER.includes(docType)) return false;
  /*
   * УПД со статусом 1 — это счёт-фактура и накладная в одном бланке, и по
   * части счёта-фактуры к нему применимо всё сказанное выше. Статус 2 —
   * только передаточный документ, там это выбор пользователя.
   */
  if (docType === 'upd' && doc && Number(doc.status) === 1) return false;
 ...
 */
function forDocument(userId, docType, doc) {
  if (!appliesTo(scopeOf(userId), docType, doc)) return {};
  const out = {};
