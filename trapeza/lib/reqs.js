...
const shorten = (name) => clean(name)
  .replace(/^индивидуальный предприниматель\b/i, 'ИП')
  .replace(/^общество с ограниченной ответственностью\b/i, 'ООО')
  .replace(/^акционерное общество\b/i, 'АО')
  .replace(/^публичное акционерное общество\b/i, 'ПАО');
