...
function parseRequisites(raw) {
  ...
  /*
   * Адрес обрывается на следующем поле блока. Список слов пришлось
   * расширить: в реквизитах после адреса часто идут ОГРН, свидетельство о
   * регистрации, почта и сайт, и без них в адрес затекал кусок соседней
   * строки — в счёте это выглядело как «дом №6 Свиде».
   */
  const NEXT = 'ИНН|КПП|ОГРНИ?П?|Свидетельств|Р\\/с|Расч|Банк|БИК|К\\/с|Корр|Тел|Факс|E-?mail|Почт|Сайт|Директор|Руководител';
  const addr = new RegExp(`Адрес\\s*[:]?\\s*([^:]+?)\\s*(?:${NEXT}|$)`, 'i').exec(text);
  if (addr) out.address = clean(addr[1]).replace(/[,;]\s*$/, '');
