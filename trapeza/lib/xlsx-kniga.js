...
async function buildKnigaProdazh({ org, docs, from, to }) {
   ...
   */
  /*
   * ⚠️ Номера граф под 5% и 7% с бланком НЕ СВЕРЕНЫ.
   *
   * Ставки УСН 5% и 7% появились в книге продаж недавно, и какая из них
   * получила «а», а какая «б» (15а/15б и 18а/18б), по памяти утверждать
   * нельзя — это ровно тот случай, когда правдоподобная догадка хуже
   * признанного незнания. Проверить неоткуда: сетевая политика этой среды
   * закрывает и pravo.gov.ru, и nalog.gov.ru.
   *
   * Что делать тому, кто это читает: открыть приложение 5 к постановлению
   * № 1137 в действующей редакции и сверить две пары номеров. Если они
   * переставлены — поменять здесь местами netNo и vatNo у ставок 5 и 7,
   * больше нигде ничего не меняется.
   *
   * Почему при этом можно жить дальше: в заголовке каждой колонки написана
   * сама ставка («Стоимость продаж без НДС, 5%»), и бухгалтер разносит по
   * ней, а не по букве. Номер графы — подсказка для сверки с бланком,
   * и ошибка в нём стоит недоразумения, а не неверной декларации.
   * У остальных ставок номера бесспорны и проверены.
   */
  const RATE_COL = {
