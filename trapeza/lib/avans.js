...
function buildAvansHtml({ org, cp, doc }) {
  ...
  const body = `
    ...
    <table class="items">
      <thead><tr>
        ${/*
           * Наименование — графа 1а, а не 1.
           *
           * С 1 июля 2021 года в бланке счёта-фактуры первой графой идёт
           * «№ п/п», а наименование сдвинулось в 1а (постановление № 1137 в
           * действующей редакции). Номер графы в шапке — не украшение: по
           * нему бухгалтер сверяет бланк с формой.
           */''}
        <th>Наименование товара (описание работ, услуг) <div class="gr">1а</div></th>
