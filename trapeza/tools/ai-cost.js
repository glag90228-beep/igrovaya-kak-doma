...
/*
 * Длину подсказки берём из самого кода, а не числом.
 *
 * Число тут устаревает молча: подсказку правят, а считалка продолжает
 * показывать вчерашнюю цену — и решение «кэшируется или нет» принимается по
 * несуществующим данным. Уже один раз так и вышло.
 *
 * Символы в токены переводим по боевому замеру: 2922 символа (подсказка плюс
 * короткая фраза) дали 1008 токенов у gemini-3.6-flash. Это всё ещё
 * пересчёт, а не замер, — но пересчёт по известному коэффициенту, а не по
 * выдуманному «делить на 2.5».
 */
const CHARS_PER_TOKEN = arg('cpt', 2922 / 1008);
const { SYSTEM } = require('../lib/ai-agent');
const SYSTEM_TOKENS = arg('system', Math.round(SYSTEM.length / CHARS_PER_TOKEN));
