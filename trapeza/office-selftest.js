...
(async () => {
  ...
  console.log('\n── база ответов по налогам ──');
  {
    ...
    /*
     * «Превысил лимит» задевает обе записи сразу: в одной есть «лимит», в
     * другой «превыс». Человек в этот момент спрашивает не про размер порога,
     * а про то, что теперь будет, — и порядок записей должен это учитывать.
     * Проверка падает, если psn-lost уедет ниже psn-limit.
     */
    ok(kn.lookup('слетел с патента что делать').entry.id === 'psn-lost', 'утрата — своя запись');
    ok(kn.lookup('превысил лимит на патенте').entry.id === 'psn-lost',
      'на «превысил лимит» отвечаем последствиями, а не размером порога');

    /*
     * Две вещи, которых в записи про лимит не было, а ответ без них неверен:
     * лимит смотрят и по прошлому году, и доходы на УСН к патентным
     * прибавляются. Без первого человек считает, что чист, пока не перебрал
     * в этом году; без второго — что упрощёнка живёт отдельно.
     */
    const lim = kn.lookup('какой лимит на патенте').entry.answer;
    ok(/прошл|2025 год/.test(lim), 'в лимите сказано, что смотрят и прошлый год');
    ok(/усн|упрощ/i.test(lim), 'и что доходы на УСН считаются вместе с патентными');
    ...
    // Одной проверкой по всем, а не ok(true) после цикла: та строка
    // рапортовала «есть» даже тогда, когда цикл выше нашёл нарушения.
    const bare = kn.ENTRIES.filter((e) => {
      const out = kn.render({ entry: e, year: null, covers: true });
      return !/Основание:/.test(out) || !/не консультация/.test(out);
    });
    ok(kn.ENTRIES.length > 0 && bare.length === 0, 'в каждом ответе базы есть норма и оговорка',
      bare.map((e) => e.id).join(', '));
