...
Скрипт забирает свежий код ветки в `/root/trapeza-src`, раскладывает его в
черновую папку `/opt/trapeza.next` и прогоняет там `npm test` — около двух
минут. Только если проверки прошли, код копируется в `/opt/trapeza` и
перезапускаются все три службы: `trapeza-bot`, `trapeza-miniapp`,
`trapeza-lava`. Упали проверки — боевая папка не тронута и работает прежняя
версия; порядок именно такой, потому что службы подгружают код с диска по
ходу работы.

В конце скрипт сам проверяет, поднялись ли службы, и для упавшей показывает
хвост её лога. Проверить руками:

```bash
systemctl status trapeza-bot trapeza-miniapp trapeza-lava --no-pager | grep -E "●|Active"
curl -s http://127.0.0.1:8788/health; echo     # приёмник оплат отвечает «ok»
```

`.env`, `data/` и `node_modules` при обновлении не трогаются. Зависимости
переустанавливаются, только если изменился `package-lock.json`. Сайт
обновляется отдельно: `bash /opt/trapeza/deploy/site.sh`.

`update.sh` копирует код поверх и ничего не удаляет: файл, который убрали из
репозитория, на сервере останется. Привести `/opt/trapeza` в точное
соответствие с репозиторием умеет `install.sh` — он раскладывает через
`rsync --delete` и сам перезапускает службы. Проверок он не гоняет, поэтому
запускать его стоит после успешного `update.sh`: тогда в `/root/trapeza-src`
лежит ровно тот код, который только что прошёл тесты.

```bash
cd /root/trapeza-src/trapeza && bash deploy/install.sh
```

Правило одно: **правьте код в репозитории, а не на сервере.** Изменение,
внесённое прямо в `/opt/trapeza`, следующее обновление перезапишет.
