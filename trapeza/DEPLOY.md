...
Раскладка идёт через `rsync --delete`: `/opt/trapeza` приводится в точное
соответствие с репозиторием, и файл, которого в git больше нет, с сервера
удаляется. Иначе положенный руками или оставшийся от прошлой версии файл
живёт там вечно, и по папке уже не понять, какой код работает.

Не трогаются только вещи, которых в git нет и быть не должно: `data/` (база,
журналы), `node_modules`, `.env` и его копии `.env.bak.*`, служебный
`.package-lock.deployed`. Зависимости переустанавливаются, только если
изменился `package-lock.json`. Сайт обновляется отдельно:
`bash /opt/trapeza/deploy/site.sh`.

Отсюда правило: **правьте код в репозитории, а не на сервере.** Изменение,
внесённое прямо в `/opt/trapeza`, следующее обновление сотрёт.
