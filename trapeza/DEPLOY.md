...
`.env`, `data/` и `node_modules` при обновлении не трогаются. Всё остальное в
`/opt/trapeza` приводится в точное соответствие с репозиторием: файлы, которых
в git нет, удаляются. Так сделано намеренно — иначе положенный руками или
оставшийся от прошлой версии файл живёт на сервере вечно, и по папке уже не
понять, какой код там работает.
