...
/*
 * Эта админка — от старой «Трапезы», и пользователь у неё ровно один.
 *
 * Её запросы к contractors и operations идут без user_id: она писалась,
 * когда база принадлежала одной компании. Но «Первичка» живёт в тех же
 * таблицах, только с колонкой user_id, и запущенная поверх её базы эта
 * админка отдаёт карточки, сальдо и всю историю операций ЛЮБОГО клиента
 * ЛЮБОГО пользователя, а по ним ещё и собирает акт сверки.
 *
 * Службой она не поднята (в deploy/ только bot, miniapp, lava и бэкап), но
 * в package.json стоит "main": "server.js" и "start": "node server.js" —
 * то есть обычный `npm start` в каталоге продукта поднимает её на 3000.
 * Проверка ниже это и ловит: увидела таблицу «Первички» — не стартуем.
 */
const multiTenant = db.prepare(
  "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'bot_users'",
).get();
if (multiTenant) {
  console.error('Это база «Первички» (многопользовательская), а server.js — админка старой '
    + '«Трапезы»: она читает контрагентов и операции без учёта владельца и показала бы '
    + 'чужие данные.\nНужен бот — запускайте `npm run bot`, приложение — `npm run miniapp`.');
  process.exit(1);
}
