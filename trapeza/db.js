...
/*
 * Таймаут ожидания — ПЕРВОЙ строкой, до всех остальных настроек.
 *
 * В базу пишут три процесса сразу — бот, мини-приложение и приёмник
 * платежей. WAL разводит чтение с записью, но две записи всё равно
 * встречаются, и без таймаута вторая падает мгновенно: «database is locked».
 *
 * Порядок тут не стилистика. Раньше таймаут стоял третьим, после
 * `journal_mode = WAL`, — а эта команда сама требует монопольной блокировки
 * и сама же в неё упирается. Получалось, что защита включалась после того
 * места, ради которого была написана.
 *
 * Как это выглядело в бою. update.sh перезапускает три службы разом, все три
 * открывают базу в одну секунду. Если предыдущие были убиты и оставили
 * неприбранный WAL, SQLite начинает его восстановление под монопольной
 * блокировкой, и остальные получают «database is locked» с нулевым
 * ожиданием. Мини-приложение падало на старте, через пять секунд
 * перезапускалось (к тому моменту восстановление закончено) и работало
 * дальше — так что в глаза бросался только испуг в выводе обновления.
 *
 * Воспроизводится устойчиво: оставить неприбранный WAL и открыть базу
 * восемью процессами разом — без этой перестановки падают двое-четверо.
 */
