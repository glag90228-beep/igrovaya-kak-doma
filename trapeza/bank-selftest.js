...
console.log('\n── книга учёта доходов ──');
{
  ...
  /*
   * Ошибки, найденные ревью.
   *
   * «Возврат аванса по договору» содержит «по договору» и становился
   * выручкой — налог с собственных денег. УФК по одному названию считалось
   * возвратом налога, и субсидия выпадала из книги. Комиссия банка без
   * назначения забивала список вопросов.
   */
  ok(kindOf('Возврат аванса по договору 5') === 'ask', '«возврат аванса по договору» — не выручка');
  ok(kindOf('УФК по Удмуртской Республике, субсидия на развитие') === 'ask',
    'субсидия из казначейства не выпадает из книги молча');
  ok(kindOf('УФК по г. Москве') === 'ask', 'казначейство без пояснений — спрашиваем');
  ok(kindOf('Возврат налога по решению УФК по г. Москве') === 'not_income',
    'а возврат налога через УФК — по-прежнему не доход');
  ok(kindOf('Грант на развитие малого бизнеса') === 'ask', 'грант — спрашиваем');
  ok(kindOf('Перечисление выручки по договору комиссии 3') === 'ask',
    'по договору комиссии доходом может быть не вся сумма — спрашиваем');
  ok(kudir.classify(row('', { incoming: false })).kind === 'not_income',
    'расход без назначения (комиссия банка) — не вопрос');
  ok(kudir.classify(row('Возврат денежных средств по договору 5', { incoming: false })).kind === 'ask',
    'неясный возврат с нашего счёта — спрашиваем, мог уйти покупателю');
  ok(kudir.classify(row('Возврат займа по договору 2', { incoming: false })).kind === 'not_income',
    'возврат займа — не уменьшение дохода');
...
console.log('\n── книга учёта: какую и кому ──');
{
  ...
  const IP = { inn: '183112345678' };
  ok(f({ ...IP, tax_mode: 'psn' }).mode === 'psn', 'патент — своя книга доходов');
  // Патент — только у предпринимателя (ст. 346.43 НК).
  ok(!f({ inn: '7707083893', tax_mode: 'psn' }).mode
    && f({ inn: '7707083893', tax_mode: 'psn' }).needs === 'tax_mode',
    'у организации патента не бывает — книгу по нему не собираем, просим поправить режим');
  ...
  ok(!f({ ...IP, tax_mode: 'psn', vat_rate: '22' }).mode, 'патент со ставкой НДС — похоже на совмещение');
  ok(/совмещ/i.test(f({ ...IP, tax_mode: 'psn', vat_rate: '22' }).blocked), 'и так и сказано');
  ...
  // Что кнопки книги и режима действительно работают, проверяет путь
  // «файл → кнопка → таблица» в bot-selftest.js («книга учёта: от файла до
  // таблицы»). Здесь раньше стоял поиск строчки в bot.js, и он был зелёным,
  // пока сама кнопка сбрасывала выписку.
}
...
console.log('\n── книга учёта: форма ФНС ──');
{
  ...
  (async () => {
    ...
    // Два неразнесённых — «2 поступления», а не «2 поступлений».
    const two = await buildKudir({ org, year: 2026, mode: 'usn',
      book: { rows: [], ask: [{ date: '2026-01-01', amount: 1, reason: 'x' }, { date: '2026-01-02', amount: 2, reason: 'y' }] } });
    const wTwo = new ExcelJS.Workbook();
    await wTwo.xlsx.load(two);
    ok(/2 поступления$/.test(String(wTwo.getWorksheet('Не разнесено').getCell('A1').value)),
      'число неразнесённых согласовано по-русски', wTwo.getWorksheet('Не разнесено').getCell('A1').value);

    // Закреплена строка над всей таблицей: у УСН — только заголовок, шапок там четыре.
    ok(wb.getWorksheet('Раздел I').views[0].ySplit === 1, 'у УСН закреплён только заголовок',
      wb.getWorksheet('Раздел I').views[0].ySplit);

    // КПП на титуле — у организации; у предпринимателя его нет.
    const ooo = await buildKudir({ org: { full_name: 'ООО «Альфа»', inn: '7707083893', kpp: '770701001' },
      year: 2026, mode: 'usn', book: { rows: [], ask: [] } });
    const wOoo = new ExcelJS.Workbook();
    await wOoo.xlsx.load(ooo);
    const titleText = [];
    wOoo.getWorksheet('Титульный лист').eachRow((r) => titleText.push(r.values.join(' ')));
    ok(titleText.some((t) => /КПП/.test(t) && /770701001/.test(t)), 'у организации на титуле есть КПП');
    const titleIp = [];
    wb.getWorksheet('Титульный лист').eachRow((r) => titleIp.push(r.values.join(' ')));
    ok(!titleIp.some((t) => /КПП/.test(t)), 'а у предпринимателя КПП нет');
